...
async function handleContextBoundaryCheck(args, mcpAuth) {
  try {
    const { content, operation = 'general', contentType = 'text', model = null } = args;

    // SEC-US1: Pass model to scopeContext for per-model context scoping
    const response = await axios.post(
      `${config.COMMAND_CENTER_API_URL}/api/v1/security/scope`,
      {
        ...
        content,
        operation,
        model, // SEC-US1: Per-model scoping differentiation
      },
      {
        headers: {
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          ...
          modelScopingTier: model ? (scoped.modelScopingTier || 'per-model') : 'default',
          redactionStats: scoped.redactionStats || {
            totalRedactions: 0,
            patternsRedacted: []
          },
          customPiiPatternsApplied: scoped.customPiiPatternsApplied || [],
          companyId: mcpAuth.companyId,
